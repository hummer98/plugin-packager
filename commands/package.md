...
- `plugin.json` の必須フィールド・型チェック
- `marketplace.json` の必須フィールド・型チェック
- `marketplace.json` の `plugins[].source` が `"./"` であること（`"."` は `source: Invalid input` になるため不可）
- `author` がオブジェクト形式であること
